...
def nn_p3p4p5(img_input=None,offset=True, num_scale=1, trainable=False):
    ...
    x = conv_block(stage5, 3, [512, 512, 4096], stage=6, block='a', strides=(1, 1), dila=(2, 2),
                   trainable=trainable)
    x1 = identity_block(x, 3, [512, 512, 4096], stage=6, block='b', dila=(1, 1), trainable=trainable)
    x2 = identity_block(x, 3, [512, 512, 4096], stage=6, block='b', dila=(2, 2), trainable=trainable)
    x1 = Conv2D(256, (1, 1), name='fpn_c6p6')(x1)
    x = Add(name="fpn_p5add")([x2,Conv2D(256, (1, 1), name='fpn_c5p5')(x1)])
